...
function themeConfig($form) {
	...
	// Twitter / X
	$socialTwitter = new Typecho_Widget_Helper_Form_Element_Text('socialTwitter', NULL, NULL, _t('页底：X'), _t('X 用户页地址（aka: Twitter）'));
...
function parseContent($obj) {
	...
	$obj->content = preg_replace('/<\s*img[\s\S]+?(?:src=[\'"]([\S\s]*?)[\'"]\s*|alt=[\'"]([\S\s]*?)[\'"]\s*|[a-z]+=[\'"][\S\s]*?[\'"]\s*)+[\s\S]*?>/i','<img src="https://cdn.jsdelivr.net/gh/yanranxiaoxi/Simplecho@0.1.19/images/lazyload.jpg" alt="$2" data-src="$1" />', $obj->content);
